...
| 이름 | 역할 | 핵심 키워드 |
| --- | --- | --- |
| 🤖 이규경 | PM / 백엔드 엔지니어 | • 프로젝트 목표·범위 정의 및 우선순위 설정
• 프로젝트 구조 설계 |
| 🤖 박대엽 | 백엔드 엔지니어 | • LLM 및 품질 게이트(테스트, 보안 등) 연계를 통해 안정적인 자동 실행 루프 구현 |
| 🤖 배유진 | 프론트엔드 엔지니어 | • 자연어 명령 입력, 진행 상태, 코드 변경 내역을 한눈에 보여주는 DAACS UI 설계
• 채팅 인터페이스, 코드 뷰어, 진행 상태 표시 등 핵심 화면 프로토타입 구현 |
| 🤖 현금비 | GitHub · 일정·문서 담당 | • GitHub 저장소 구조, 브랜치 전략, 이슈·PR 프로세스 설계를 통한 작업 흐름 정리
• 기획서·설계서·회의록 등 주요 산출물 관리 및 일정·진행 상황 가시성 확보 |
