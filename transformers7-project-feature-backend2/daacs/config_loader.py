...
class DAACSConfig:
    ...
    def __init__(self, config_path: str = "daacs_config.yaml"):
        ...
        # 역할별 CLI 타입 (기본값은 전역 cli_type)
        self.role_cli_types: Dict[str, str] = {
            "orchestrator": self.cli_type,
            "backend": self.cli_type,
            "frontend": self.cli_type
        }
    ...
    def _create_llm_sources(self):
        ...
        for role in ["orchestrator", "backend", "frontend"]:
            ...
            if role_config:
                try:
                    # 역할별 CLI 타입 사용
                    role_cli_type = self.role_cli_types.get(role, self.cli_type)
